...
void getbetter2(int start,int end)
{
	...
	for (i=start;i<=end;i++)
	{
		...
		if (gb[i].size() == 3 && checkdigit(gb[i][2])) 
		{
			...
			constant.insert(make_pair( gb[i][0] ,ll ));
		}
		if (gb[i].size() == 3)
		{
			...
			{
				if (con->first == gb[i][2])
					gb[i][2] = to_string(long double(con->second));
			}
		}
		if (gb[i].size() == 5) 
		{
			...
			for (con = constant.begin(); con != constant.end(); con++)
			{
				...
				if (con->first == gb[i][4])
				{
					gb[i][4] = to_string(long double(con->second));
					flag++;
				}
			}
			if (flag == 2)
			{
				...
				{
					case 1:
						gb[i][2] = to_string(long double(b + d));
						break;
					case 2:
						gb[i][2] = to_string(long double(b - d));
						break;
				...
				gb[i].pop_back();
				gb[i].pop_back();
				constant[gb[i][0]] = atol(gb[i][2].c_str());
			}
